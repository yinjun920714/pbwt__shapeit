  ...
  while (argc) {
	...
	paintAncestryMatrixSparse (p, argv[1],npr,0) ; argc -= 2 ; argv += 2 ; 
      ...
      {         
        ...
        pbwtShapeIt1 (p, out) ; argc -= 2 ; argv += 2 ; }
    else if (!strcmp (argv[0], "-shapeIt2") && argc > 2)
      {         
        FILE *out = fopen(argv[2], "w");
        pbwtShapeIt2 (p, atoi(argv[1]), out) ; argc -= 3 ; argv += 3 ; }
    ...
    else if (!strcmp (argv[0], "-shapeItWithMiss") && argc > 2)
      { 
        FILE *out = fopen(argv[2], "w");
        pbwtShapeItWithMiss (p, atoi(argv[1]), out) ; argc -= 3 ; argv += 3 ; }
    else
