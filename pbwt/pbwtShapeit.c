...
void pbwtMatchCount2 (PBWT *p, FILE *fp, int maxGeno, FILE *out) //extensibile heter number
{
  if (!p || !p->yz) die ("option -longWithin called without a PBWT") ;
...
void pbwtMatchCount3 (PBWT *p, FILE *fp, FILE *out, int percent) //combine the pbwt and sub-pbwt
{
  if (!p || !p->yz) die ("option -longWithin called without a PBWT") ;
  uchar **reference = pbwtHaplotypes (p) ; /* haplotypes for reference  (M * N)  */
  /***************** pbwt part *******************/
  ...
  int num_1; /* for the num of 1 */     
  double w = 1.0 / M;
  double coefficent = (double)percent / 100;
  ...
  for (t = 0; t < TIMES; ++t) {
    ...
    //minus the f1,g1, subf1, subg1 by 1 for the origin sequence
    for ( i = 0; i < seg_num - 1; ++i) {
      int index = origin[0][pos[i * 3]] * 4 + origin[0][pos[i * 3 + 1]] * 2 + origin[0][pos[i * 3 + 2]];
      f1[index][i]++;      //for origin[0];
      f1[7 - index][i]++;  //for origin[1];
      subf1[index][i]++;      //for origin[0];
      subf1[7 - index][i]++;  //for origin[1];
    }
    ...
    for ( i = 0; i < seg_num - 2; ++i) {
      ...
      f2[index][i]++;      //for origin[0];
      f2[63 - index][i]++;  //for origin[1];
      subf2[index][i]++;      //for origin[0];
      subf2[63 - index][i]++;  //for origin[1];
    }
...
//combine the whole pbwt and sub-pbwt
